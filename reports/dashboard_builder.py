...
function rafExpandRow(el,innerSel,duration,onDone){
  ...
  if(inner){inner.style.opacity='0';inner.style.transform='translateY(-8px)';inner.style.transition='none';}
  const toH=el.scrollHeight;
  const start=performance.now();
  function frame(now){
    const raw=Math.min((now-start)/duration,1),ease=easeOutQuart(raw);
    el.style.height=(toH*ease).toFixed(1)+'px';
    if(inner){
      inner.style.opacity=Math.min(raw*2,1).toFixed(3);
      inner.style.transform=`translateY(${(-8*(1-ease)).toFixed(2)}px)`;
    }
    if(raw<1){requestAnimationFrame(frame);}
    else{
      el.classList.add('open');
      el.style.height=''; el.style.overflow='';
      if(inner){inner.style.opacity='';inner.style.transform='';inner.style.transition='';}
      if(onDone)onDone();
    }
  }
...
function rafCollapseRow(el,innerSel,duration,onDone){
  ...
  function frame(now){
    ...
    if(inner){
      inner.style.opacity=(1-Math.min(raw*2.5,1)).toFixed(3);
      inner.style.transform=`translateY(${(-6*ease).toFixed(2)}px)`;
    }
    if(raw<1){requestAnimationFrame(frame);}
    else{
